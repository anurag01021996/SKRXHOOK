...
SDL_GLContext SKRXHOOK_context = nullptr;
...
void SDL2::SwapWindow(SDL_Window* window)
{
	...
	if (!SKRXHOOK_context)
	{
		SKRXHOOK_context = SDL_GL_CreateContext(window);
	...
	SDL_GL_MakeCurrent(window, SKRXHOOK_context);
...
void SDL2::UnhookWindow()
{
	*swapWindowJumpAddress = oSwapWindow;
	...
	SDL_GL_DeleteContext(SKRXHOOK_context);
}
